...
int cnt(string & nums,int n,int tight){
	...
	if(n==0) return 1; // number equal to R as tight = 1
	int ct=0;
	int ub=tight?nums[nums.size()-n]-'0':9;
	for(int i=0;i<=ub;++i){
		ct+=cnt(nums,n-1,tight&(i==ub));
	}
	return ct;
}
int dp[20][2];
int f(string & nums,int n,int tight){
	if(n==0) return 0;
	if(dp[n][tight]!=-1) return dp[n][tight];
	int ans=0;
	int ub=tight?nums[nums.size()-n]-'0':9;
	for(int i=0;i<=ub;++i){
		ans+=i*cnt(nums,n-1,tight&(i==ub));
		ans+=f(nums,n-1,tight&(i==ub));
	}
...
int32_t main(){
    ...
    while(t--){
    	...
    	int lower=f(L,L.size(),1);
    	memset(dp,-1,sizeof dp);
    	int upper=f(R,R.size(),1);
    	cout<<upper-lower<<'\n';
    }
}
