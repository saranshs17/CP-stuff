...
//<---------------------------code begins--------------------------->
int dp[19][100005][2];
int f(int n,string &num,int tar,int tight){
	if(n==0){
		if(tar==0)return 1;
		else return 0;
	}
	if(dp[n][tar][tight]!=-1) return dp[n][tar][tight];
	int index=num.size()-n;
	int ub = tight?num[index]-'0':9;
	int ans=0;
	for(int i=0;i<=ub;++i){
		ans+=f(n-1,num,tar-i,tight&(i==ub));
	}
	return dp[n][tar][tight]=ans;
}
int32_t main(){
    //sieve();
    // SPF();
    ...
    while(t--){
    	...
    	r=to_string(b);
    	int tar;cin>>tar;
    	cout<<f(r.size(),r,tar,1)-f(l.size(),l,tar,1)<<'\n';
    }
}
