...
//<---------------------------code begins--------------------------->
int dp[20][2][10][2];
int f(string & nums,int n,int leading,int prev,int tight){
	if(n==0) return 1;
	if(prev!=-1 && dp[n][leading][prev][tight]!=-1) return dp[n][leading][prev][tight];
	int ub=(tight==1)?nums[nums.size()-n]-'0':9;
	int ans=0;
	if(leading == 0){
		for(int i=0;i<=ub;++i){
			if(i==prev)continue;
			ans+=f(nums,n-1,0,i,tight&(i==ub));
		}
	}else{
		for(int i=0;i<=ub;++i){
			if(i==prev && i!=0)continue;
			if(i==0) ans+=f(nums,n-1,1,i,tight&(i==ub));
			else ans+=f(nums,n-1,0,i,tight&(i==ub));
		}
	}
	return dp[n][leading][prev][tight]=ans;
}
int32_t main(){
    //sieve();
    // SPF();
    ...
    int t;
   	t=1;
    while(t--){
    	// dp[n][leading][prev][tight]
    	int l,r;cin>>l>>r;
    	l--;
    	string L=to_string(l);
    	string R=to_string(r);
    	memset(dp,-1,sizeof dp);
    	int ansL=f(L,L.size(),1,-1,1);
    	memset(dp,-1,sizeof dp);
    	int ansR=f(R,R.size(),1,-1,1);
    	cout<<ansR-ansL<<'\n';
    }
}
