...
int f(string &nums,int n,int even,int leadingzero,int tight){
	if(n==0)return 1;
	int ans=0;
	if(even){
		int ub=tight?nums[nums.size()-n]-'0':8;
		for(int i=0;i<=ub;i+=2){
			ans+=f(nums,n-1,0,0,tight&(i==ub));
		}
	}else{
		// can put 0 at current index
		if(leadingzero){
			ans+=f(nums,n-1,0,1,0);
		}
		// or can stop putting leading zero and start sequence 
		int ub=tight?nums[nums.size()-n]-'0':9;
		for(int i=1;i<=ub;i+=2){
			ans+=f(nums,n-1,1,0,tight&(i==ub));
		}
	}
	return ans;
}
int32_t main(){
    ...
    while(t--){
    	...
    	l=to_string(L);
    	r=to_string(R);
    	cout<<f(r,r.size(),0,1,1)-f(l,l.size(),0,1,1)<<'\n';
    }
}
